/*******************************************************************************
 ...
 Modified by Dave Klimas for testing the HyperDuino & Adafruit MPR121 breakout boards
 Disabled interrupt function to free up pin
...
MPR121_t::MPR121_t(){
	Wire.begin();	
	address = 0x5A;   // default address is 0x5A for HyperDuino & adafruit board 
                    // 0x5C for use with Bare Touch Board
	ECR_backup = 0x00;
	running = false;
...
void MPR121_t::applySettings(MPR121_settings_t *settings){
  ...
  // free up this pin (disable interrupt)
...
/*
	...
	for (uint8_t i=0; i<0x7F; i++) {
		...
		Serial.print("Reg:" + str);
		str = String(getRegister(i),DEC);
		while(str.length() < 3 ) str = '0' + str;
		// print decimal register value
		Serial.print(" " + str);
		str = String(getRegister(i),BIN);
		while( str.length() < 8 ) str = '0' + str;
		// print binary register value
		Serial.println("-" + str);
	}
*/
}
...
bool MPR121_t::touchStatusChanged(){
  ...
  // free up this pin (disable interrupt)
	...
	return(autoTouchStatusFlag || true);
}

/*
  ...
  Hyperduino:
  We use this i2c library because it is guaranteed to be lock free, basically
  provices noise resistance on i2c bus. This is important as we share the bus
  as an analog input.
*/
...
uint8_t I2C_hf::bytesAvailable = 0;
uint8_t I2C_hf::bufferIndex = 0;
uint8_t I2C_hf::totalBytes = 0;
uint16_t I2C_hf::timeOutDelay = 0;
...
void I2C_hf::timeOut(uint16_t _timeOut)
{
  timeOutDelay = _timeOut;
}

void I2C_hf::setSpeed(uint8_t _fast)
{
  if(!_fast)
  {
    TWBR = ((F_CPU / 100000) - 16) / 2;
...
void I2C_hf::pullup(uint8_t activate)
{
  if(activate)
  {
    #if defined(__AVR_ATmega168__) || defined(__AVR_ATmega8__) || defined(__AVR_ATmega328P__)
...
uint8_t I2C_hf::available()
{
  return(bytesAvailable);
}

uint8_t I2C_hf::receive()
{
...
uint8_t I2C_hf::write(uint8_t address, uint8_t registerAddress)
{
  returnStatus = 0;
  returnStatus = start();
...
uint8_t I2C_hf::write(int address, int registerAddress)
{
  return(write((uint8_t) address, (uint8_t) registerAddress));
}

uint8_t I2C_hf::write(uint8_t address, uint8_t registerAddress, uint8_t data)
{
  returnStatus = 0;
  returnStatus = start(); 
  if(returnStatus){return(returnStatus);}
...
uint8_t I2C_hf::write(int address, int registerAddress, int data)
{
  return(write((uint8_t) address, (uint8_t) registerAddress, (uint8_t) data));
}

uint8_t I2C_hf::write(uint8_t address, uint8_t registerAddress, char *data)
{
...
uint8_t I2C_hf::write(uint8_t address, uint8_t registerAddress, uint8_t *data, uint8_t numberBytes)
{
  returnStatus = 0;
  returnStatus = start();
  if(returnStatus){return(returnStatus);}
...
uint8_t I2C_hf::read(int address, int numberBytes)
{
  return(read((uint8_t) address, (uint8_t) numberBytes));
}

uint8_t I2C_hf::read(uint8_t address, uint8_t numberBytes)
{
  bytesAvailable = 0;
  bufferIndex = 0;
...
/* uint8_t I2C_hf::read(int address, int registerAddress, int numberBytes)
{
  return(read((uint8_t) address, (uint8_t) registerAddress, (uint8_t) numberBytes));
}*/

uint8_t I2C_hf::read(uint8_t address, uint8_t registerAddress, uint8_t numberBytes)
{
  bytesAvailable = 0;
  bufferIndex = 0;
  if(numberBytes == 0){numberBytes++;}
...
uint8_t I2C_hf::read(uint8_t address, uint8_t numberBytes, uint8_t *dataBuffer)
{
  bytesAvailable = 0;
  bufferIndex = 0;
  if(numberBytes == 0){numberBytes++;}
...
uint8_t I2C_hf::read(uint8_t address, uint8_t registerAddress, uint8_t numberBytes, uint8_t *dataBuffer)
{
  bytesAvailable = 0;
  bufferIndex = 0;
  if(numberBytes == 0){numberBytes++;}
...
uint8_t I2C_hf::sendAddress(uint8_t i2cAddress)
{
  TWDR = i2cAddress;
  unsigned long startingTime = millis();
  TWCR = (1<<TWINT) | (1<<TWEN);
...
uint8_t I2C_hf::sendByte(uint8_t i2cData)
{
  TWDR = i2cData;
  unsigned long startingTime = millis();
  TWCR = (1<<TWINT) | (1<<TWEN);
...
uint8_t I2C_hf::receiveByte(uint8_t ack)
{
  unsigned long startingTime = millis();
  if(ack)
  {
